mod fernando;
use std::{env::{self, args}, io::{stdin, Read}};
...
fn main() {
    ...
    // Blocos (mini funções que não precisam de declaração dentro de outra função)
    let var = {
        let pi = 3.1415;
        let r = 2.0;
        pi * r * r
    ...
    let arg1 = &args[1];
    let arg2 = &args[2];
    println!("Primeiro argumento {arg1}");
    println!("Segundo argumento {arg2}");
     */

    // Via std::io::stdin
    let mut n = String::new();
    stdin().read_line(&mut n).expect("Input failed");
    let n:f32 = n.trim().parse().expect("invalid input"); // transforma string em f32
    println!("{}", n);
    fernando::fernandin();
    println!("Illana = {}", fernando::illana(30, 20));
